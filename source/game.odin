...
update :: proc() {
    ...
    if linalg.length(input) > 0 {
        ...
        if current_animation.current_frame >=
           current_animation.num_frames - 1 {
    ...
    for npc, handle_npc in ha_iter_ptr(&npcs_iter) {
        ...
        if distance_to_player < g.player.distance_nearest_npc &&
           distance_to_player < TALK_MAX_DISTANCE {
...
draw :: proc() {
    ...
    if g.player.distance_nearest_npc < TALK_MAX_DISTANCE {
        ...
        if g.player.talking {
            ...
            draw_texture_pos(
                g.icons.speech_bubble.texture,
                (nearest_npc.pos + {0.0, -30.0}),
                g.icons.speech_bubble.z,
            )
        }
    }
...
game_init :: proc() {
    ...
    g^ = Game_Memory {
        ...
        player = Player {
            ...
            idle_down = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Idle_Base/Idle_Down-Sheet.png",
                ),
                4,
            ),
            idle_up = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Idle_Base/Idle_Up-Sheet.png",
                ),
                4,
            ),
            idle_right = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Idle_Base/Idle_Side-Sheet.png",
                ),
                4,
            ),
            walk_down = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Walk_Base/Walk_Down-Sheet.png",
                ),
                6,
            ),
            walk_up = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Walk_Base/Walk_Up-Sheet.png",
                ),
                6,
            ),
            walk_right = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Walk_Base/Walk_Side-Sheet.png",
                ),
                6,
            ),
            melee_slice_down = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Slice_Base/Slice_Down-Sheet.png",
                ),
                8,
            ),
            melee_slice_up = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Slice_Base/Slice_Up-Sheet.png",
                ),
                8,
            ),
            melee_slice_right = animation_create(
                rl.LoadTexture(
                    "assets/Entities/Characters/Body_A/Animations/Slice_Base/Slice_Side-Sheet.png",
                ),
                8,
            ),
        },
        tree_texture = rl.LoadTexture(
            "assets/Environment/Props/Static/Trees/Model_01/Size_05.png",
        ),
        icons = Icons {
            speech_bubble = Icon {
    ...
    knight := NPC {
        animation = animation_create(
            rl.LoadTexture("assets/Entities/Npc's/Knight/Idle/Idle-Sheet.png"),
            4,
        ),
        pos       = {-80, 10},
    }
